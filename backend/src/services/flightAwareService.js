...
// ── Filtrado ──────────────────────────────────────────────────
// ── Helpers de filtrado ───────────────────────────────────────
function obtenerFechaVuelo(f, mode) {
  return fechaLocal(
    mode === "departures" ? f.scheduledOut : (f.scheduledIn || f.scheduledOut)
  );
}

function cumpleFiltroTipo(f, type) {
  return !type || type === "all" || f.type === type;
}

function cumpleFiltroAerolinea(f, airlineId) {
  return !airlineId || airlineId === "all" || f.airlineId === airlineId;
}

function cumpleFiltroVuelo(f, search) {
  if (!search) return true;
  if (!f.flightNumber) return false;
  return f.flightNumber.toUpperCase().includes(search.toUpperCase());
}

function cumpleFiltroUbicacion(f, locationSearch, mode) {
...
function filtrar(vuelos, opciones, mode) {
  ...
  return vuelos.filter(f => {
    const fv = obtenerFechaVuelo(f, mode);
    if (!fv || !fechasValidas.has(fv))                   return false;
    if (date && fv !== date)                             return false;
    if (!cumpleFiltroTipo(f, type))                      return false;
    if (!cumpleFiltroAerolinea(f, airlineId))            return false;
    if (!cumpleFiltroVuelo(f, search))                   return false;
    if (!cumpleFiltroUbicacion(f, locationSearch, mode)) return false;
    return true;
  });
}
...
module.exports.obtenerFechaVuelo = obtenerFechaVuelo;
module.exports.cumpleFiltroAerolinea = cumpleFiltroAerolinea;
module.exports.cumpleFiltroVuelo = cumpleFiltroVuelo;
module.exports.cumpleFiltroUbicacion = cumpleFiltroUbicacion;
module.exports.cumpleFiltroTipo = cumpleFiltroTipo;