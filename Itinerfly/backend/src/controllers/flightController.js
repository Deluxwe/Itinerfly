...
async function getDepartures(req, res) {
  try {
    ...
    if (date && !esFechaValida(date)) return clientError(res, "Formato de fecha inválido. Usa YYYY-MM-DD.", 400);
    if (date && !getNextDays(3).includes(date)) return clientError(res, "Fecha fuera del rango permitido (máximo 2 días).", 400);
    if (type && !["domestic","international","all"].includes(type)) return clientError(res, "Tipo inválido. Usa: domestic, international o all.", 400);
...
async function searchByLocation(req, res) {
  try {
    ...
    if (!q || q.trim().length < 2) return clientError(res, "La búsqueda necesita al menos 2 caracteres.", 400);
    if (mode && !["departures","arrivals"].includes(mode)) return clientError(res, "Modo inválido. Usa: departures o arrivals.", 400);
...
async function getFlightDetail(req, res) {
  try {
    ...
    if (codigo.length < 3 || codigo.length > 8) return clientError(res, "Código de vuelo inválido.", 400);
