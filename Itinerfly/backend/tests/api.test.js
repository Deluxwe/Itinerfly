...
describe('GET /api/flights/departures', () => {
  ...
  it('filtra por tipo domestic', async () => {
    ...
    expect(res.status).toBe(200)
    res.body.data.flights.forEach(f => expect(f.type).toBe('domestic'))
  })
  it('filtra por tipo international', async () => {
    const res = await request(app).get('/api/flights/departures?type=international')
    expect(res.status).toBe(200)
    res.body.data.flights.forEach(f => expect(f.type).toBe('international'))
  })
  ...
  it('acepta fecha de hoy en formato YYYY-MM-DD', async () => {
    // Fix: usamos hoy que siempre está en rango
    const hoy = new Date().toISOString().split('T')[0]
    const res = await request(app).get(`/api/flights/departures?date=${hoy}`)
    expect(res.status).toBe(200)
  })
  it('rechaza fecha fuera del rango de 2 días', async () => {
    // Fix: una fecha pasada debe dar 400
...
describe('GET /api/routes', () => {
  ...
  it('devuelve array de rutas', async () => {
    const res = await request(app).get('/api/routes')
    // Fix: el endpoint /api/routes devuelve data.routes
